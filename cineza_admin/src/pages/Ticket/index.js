  ...
  useEffect(() => {
    const findTicket = async () => {
      const tickets = await axios.get(
        `http://54.169.84.199:9000/cineza/api/v1/ticket/get-all?movieName=${movieName}&showDate=`
      );
  ...
  const getData = async () => {
    try {
      const result = await axios.get(
        "http://54.169.84.199:9000/cineza/api/v1/ticket/get-all"
      );
    ...
    if (code != "") {
      const getTicket = async () => {
        const ticket = await axios.get(
          `http://54.169.84.199:9000/cineza/api/v1/ticket/get-by-code/${code}`
        );
        if (ticket.status === 200) {
          setSelectTicket(ticket.data);
          const allSeat = await axios.get(
            `http://54.169.84.199:9000/cineza/api/v1/seat/get-all-by-room/${ticket.data.roomCode}`
          );
