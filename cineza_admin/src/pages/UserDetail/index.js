      ...
      if (addBtn) {
        ...
        try {
          const response = await axios.get(
            `http://47.128.146.135:9000/cineza/api/v1/user/get-by-code/${codeUserBy}`
          );
  ...
  const onClickHandleSave = async () => {
    ...
    if (
      ...
      try {
        if (editCode) {
          let errorCreate = "";
          const checkEmail = await axios.get(`http://47.128.146.135:9000/cineza/api/v1/user/check-email/${phoneUser}`)
          if (checkEmail.data != "") {
            errorCreate = "Email đã tồn tại";
          }
          const response = await axios.post(
            `http://47.128.146.135:9000/cineza/api/v1/user/create`,
            user
          );
          ...
          const response = await axios.put(
            `http://47.128.146.135:9000/cineza/api/v1/user/update/${codeUser}`,
            user
          );
          if (response.status === 200) {
