...
const PromotionDetail = ({ codePromotion, onClickHandleClose, addBtn }) => {
    ...
    try {
      if (editCode) {
        const response = await axios.post(
          `http://54.169.84.199:9000/cineza/api/v1/promotion-line/create`,
          promotionLine
        );
        ...
        const responseDetail = await axios.post(
          `http://54.169.84.199:9000/cineza/api/v1/promotion-detail/create`,
          promotionDetail
        );
        ...
        //     `http://54.169.84.199:9000/cineza/api/v1/value/user/put/` + codeUser,
        //     user
        // );
        // if (response.status === 200) {
  ...
  useEffect(() => {
    const getUser = async () => {
      ...
      try {
        const response = await axios.get(
          `http://54.169.84.199:9000/cineza/api/v1/promotion-line/get-by-code/` +
          codePromotion
        );
        ...
        if (response.status === 200) {
          ...
          const responseDetail = await axios.get(
            `http://54.169.84.199:9000/cineza/api/v1/promotion-detail/get-all/` +
            response.data.code
          );
