...
const PromotionDetail = ({ codePromotion, onClickHandleClose, addBtn }) => {
    ...
    try {
      if (editCode) {
        const response = await axios.post(
          `http://13.212.34.123:9000/cineza/api/v1/promotion-line/create`,
          promotionLine
        );
        ...
        const responseDetail = await axios.post(
          `http://13.212.34.123:9000/cineza/api/v1/promotion-detail/create`,
          promotionDetail
        );
  ...
  useEffect(() => {
    const getUser = async () => {
      ...
      try {
        const response = await axios.get(
          `http://13.212.34.123:9000/cineza/api/v1/promotion-line/get-by-code/` +
          codePromotion
        );
        ...
        if (response.status === 200) {
          ...
          const responseDetail = await axios.get(
            `http://13.212.34.123:9000/cineza/api/v1/promotion-detail/get-all/` +
            response.data.code
          );
    ...
    const getAllPromotionHeader = async () => {
      try {
        const allPromotionHeader = await axios.get(
          "http://13.212.34.123:9000/cineza/api/v1/promotion-header/get-all"
        );
