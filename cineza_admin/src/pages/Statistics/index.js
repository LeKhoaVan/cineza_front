...
function Statistics() {
    ...
    const handleUserSearch = async (e) => {
        ...
        try {
            if (e.target.value.trim().length > 0) {
                const resultApi = await axios.get(`http://54.169.84.199:9000/cineza/api/v1/user/find-user-by-name/${e.target.value}`)
                if (resultApi != null) {
    ...
    const handleMovieSearch = async (e) => {
        ...
        try {
            if (e.target.value.trim().length > 0) {
                const resultApi = await axios.get(`http://54.169.84.199:9000/cineza/api/v1/movie/get-all?movieName=${e.target.value}`);
                if (resultApi != null) {
    ...
    useEffect(() => {
        const getTotalOrder = async () => {
            const resultData = await axios.get(`http://54.169.84.199:9000/cineza/api/v1/statistics/get-total-order`);
            if (resultData != null) {
                let total = 0;
    ...
    useEffect(() => {
        const getTotalTicket = async () => {
            const resultData = await axios.get(`http://54.169.84.199:9000/cineza/api/v1/statistics/get-total-ticket`);
            if (resultData != null) {
                let total = 0;
    ...
    useEffect(() => {
        const getStatistics = async () => {
            ...
            const statisticsResult = await axios.get(`http://54.169.84.199:9000/cineza/api/v1/statistics/get-order-by-time-user-movie?timeStart=${stDay}&timeEnd=${edDay}&user=${selectUser}&movie=${selectedMovie}`);
            let total = 0;
    ...
    useEffect(() => {
        const getStatisticsTicket = async () => {
            const stDay = startDate == "" ? "" : moment(startDate).format("YYYY-MM-DD");
            const edDay = endDate == "" ? "" : moment(endDate).format("YYYY-MM-DD");
            const statisticsResult = await axios.get(`http://54.169.84.199:9000/cineza/api/v1/statistics//get-ticket-by-time-user-movie?timeStart=${stDay}&timeEnd=${edDay}&user=${selectUser}&movie=${selectedMovie}`);
            let total = 0;
            let totalThuong = 0;
            let totalVIP = 0;
            if (statisticsResult.data != "") {
    ...
    useEffect(() => {
        const statisticsMovie = async () => {
            const resultStatistic = await axios.get(`http://54.169.84.199:9000/cineza/api/v1/statistics/top-5-movie?startDate=${startDate}&endDate=${endDate}`)
