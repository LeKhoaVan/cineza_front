      ...
      const getAddressByCode = async () => {
        try {
          const response = await axios.get(
            `http://54.169.2.153:9000/cineza/api/v1/address/get-by-code/${codeAddressBy}`
          );
    ...
    try {
      if (editCode) {
        const response = await axios.post(
          `http://54.169.2.153:9000/cineza/api/v1/address/create`,
          address
        );
        ...
        const response = await axios.put(
          `http://54.169.2.153:9000/cineza/api/v1/address/update/${codeAddress}`,
          address
        );
        if (response.status === 200) {
          console.log("save success");
