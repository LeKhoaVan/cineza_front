    ...
    if (codeMovie != "") {
      const getDate = async () => {
        const resutlDate = await axios.get(
          `http://47.128.146.135:9000/cineza/api/v1/movie/${codeMovie}`
        );
    ...
    if (addBtn) {
      ...
      const getShow = async () => {
        const result = await axios.get(
          `http://47.128.146.135:9000/cineza/api/v1/show/get-by-code/${codeShow}`
        );
  ...
  useEffect(() => {
    const getAllTicket = async () => {
      const allTicket = await axios.get(
        `http://47.128.146.135:9000/cineza/api/v1/ticket/get-by-showing/${codeShow}`
      );
    ...
    if (codeRoom != "") {
      const getAllSeat = async () => {
        const allSeat = await axios.get(
          `http://47.128.146.135:9000/cineza/api/v1/seat/get-all-by-room/${codeRoom}`
        );
        const allTicket = await axios.get(
          `http://47.128.146.135:9000/cineza/api/v1/ticket/get-by-showing/${codeShow}`
        );
        ...
        if (editCode) {
          ...
          if (timeMovie.data != "not found!") {
            ...
            const checkTime = await axios.get(
              `http://47.128.146.135:9000/cineza/api/v1/show/check-show/${codeRap}/${codeRoom}/${dateCheck}/${showStart}/${timeEnd1}`
            );
            console.log(dateCheck);
            console.log(checkTime.data.length);
            if (checkTime.data.length === 0) {
              const response = await axios.post(
                `http://47.128.146.135:9000/cineza/api/v1/show/create`,
                show
              );
          ...
          if (timeMovie.data != "not found!") {
            ...
            const checkTime = await axios.get(
              `http://47.128.146.135:9000/cineza/api/v1/show/check-show/${codeRap}/${codeRoom}/${dateCheck}/${showStart}/${timeEnd1}?codeShow=${codeShow}`
            );

            if (checkTime.data.length === 0) {
              const response = await axios.put(
                `http://47.128.146.135:9000/cineza/api/v1/show/put/` + code,
                show
              );
