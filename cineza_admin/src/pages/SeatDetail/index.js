  ...
  useEffect(() => {
    const getSeat = async () => {
      const result = await axios.get(
        `http://13.212.34.123:9000/cineza/api/v1/seat/get-by-code/${codeSeat}`
      );
      ...
      try {
        ...
        if (editCode) {
          const response = await axios.post(
            `http://13.212.34.123:9000/cineza/api/v1/seat/create`,
            seat
          );
          ...
          const response = await axios.put(
            `http://13.212.34.123:9000/cineza/api/v1/seat/put/` + code,
            seat
          );
