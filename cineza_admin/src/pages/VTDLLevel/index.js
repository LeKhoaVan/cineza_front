    ...
    if (levelAddress != undefined) {
      try {
        const response = await axios.get(
          `http://47.128.146.135:9000/cineza/api/v1/address/get-by-level?levelAddress=${levelAddress}`
        );
      ...
      try {
        const response = await axios.get(
          `http://47.128.146.135:9000/cineza/api/v1/address/get-by-level?levelAddress=${address[0].level}`
        );
        setAddress(response.data);
      } catch (error) {
        console.error("error get address by type in VTDLLevel: " + error);
