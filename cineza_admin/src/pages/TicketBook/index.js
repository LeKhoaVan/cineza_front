...
const TicketBook = () => {
    ...
    useEffect(() => {
        const loadMovieOrRap = async () => {
            if (selectBy === "phim") {
                const movies = await axios.get(`http://localhost:9000/cineza/api/v1/movie/get-all`);
                ...
                const raps = await axios.get(`http://localhost:9000/cineza/api/v1/rap/get-all`);
                if (raps.status === 200) {
                    setAllRap(raps.data);
                    setAllMovie([])
                    setCodeMovie("")
    ...
    const handleClick = async () => {
        try {
            if (codeShowing != "" && codeMovie != "") {
                const saveTicket = await axios.post(`http://localhost:9000/cineza/api/v1/ticket/create`, {
    ...
    useEffect(() => {
        const loadRap = async () => {
            if (codeMovie != "") {
                const showings = await axios.get(`http://localhost:9000/cineza/api/v1/show/get-all-by-movie/${codeMovie}`)

                // Sử dụng Set để loại bỏ rạp trùng lặp
                // const uniqueRaps = [...showings.data?.map(async (show) => {
                //     const rap = await axios.get(`http://localhost:9000/cineza/api/v1/rap/get-by-code/${show.codeRap}`)
                //     return rap.data
                // })];
                ...
                async function fetchData() {
                    const rapPromises = showings.data?.map(async (show) => {
                        const rapResponse = await axios.get(`http://localhost:9000/cineza/api/v1/rap/get-by-code/${show.codeRap}`);
    ...
    useEffect(() => {
        const loadMovie = async () => {
            if (codeRap != "") {
                const showings = await axios.get(`http://localhost:9000/cineza/api/v1/show/get-all-by-rap/${codeRap}`)

                async function fetchDataMovie() {
                    const moviePromises = showings.data?.map(async (show) => {
                        const movieResponse = await axios.get(`http://localhost:9000/cineza/api/v1/movie/${show.codeMovie}`);
    ...
    useEffect(() => {
        if (codeMovie != "" && codeRap != "") {
            const getShowing = async () => {
                const showings = await axios.get(`http://localhost:9000/cineza/api/v1/show//get-by-movie-and-rap/${codeMovie}/${codeRap}`)
    ...
    useEffect(() => {
        if (codeShowing != "") {
            const getShowing = async () => {
                const showing = await axios.get(`http://localhost:9000/cineza/api/v1/show/get-by-code/${codeShowing}`)
                setShowing(showing.data)

                const allSeat = await axios.get(`http://localhost:9000/cineza/api/v1/seat/get-all-by-room/${showing.data.codeRoom}`)
